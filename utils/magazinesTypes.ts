...
export function formatMagazineTitle(filename: string): string {
  ...
  if (
    nameWithoutExt.includes("28july_to_03august_2025") &&
    nameWithoutExt.includes("icep") &&
    nameWithoutExt.includes("magazine")
  ) {
  ...
  const match = nameWithoutExt.match(
    /^(\d{2}\w+)_to_(\d{2}\w+)_(\d{4})-(.+)$/i
  );

  if (match) {
    const [, startDate, endDate, year, magazineName] = match;
    const cleanMagazineName = magazineName
      .replace(/-/g, " ")
